...
import { Building2, Plus, Trash2, Edit2 } from 'lucide-vue-next';
  ...
  <div class="max-w-6xl mx-auto">
    <div class="flex items-center justify-between mb-8">
      <div>
        ...
        <p class="text-gray-500 mt-1 text-sm">Manage your company departments</p>
      </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
      ...
      <table v-else class="w-full text-left text-sm text-gray-600">
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="dept in adminStore.departments" :key="dept.id" class="hover:bg-gray-50 transition-colors">
          ...
          <tr v-if="adminStore.departments.length === 0">
            <td colspan="3" class="px-6 py-8 text-center text-gray-500">No departments found. Create one to get started.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <h3 class="font-semibold text-gray-900">{{ isEditing ? 'Edit' : 'Add' }} Department</h3>
        ...
        <form @submit.prevent="handleSubmit" class="p-6">
          <div class="mb-4">
            ...
            <input v-model="form.name" type="text" required class="block w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-gray-900 focus:ring-2 focus:ring-blue-600 focus:border-transparent outline-none transition-all" placeholder="e.g. Engineering" />
          </div>
          <div class="flex justify-end gap-3 mt-6">
            ...
            <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium transition-colors">Save Department</button>
          </div>
        </form>
      </div>
    </div>
